...
public class GridContainerTest extends TestCase {
  ...
  public void testIsValidMoveForLevel1() {
    ...
    assertMoveIsValid(0, 1, true,  2, 1);
    assertMoveIsValid(0, 1, true,  2, 2);
  ...
  void assertMoveIsValid(int row, int column, boolean isValid, int toRow, int toColumn) {
    if (isValid) {
      assertTrue(grid.isValidMove(
                   grid.cell(row, column).square(),
                   grid.cell(toRow, toColumn)));
    }
    else {
      assertFalse(grid.isValidMove(
                    grid.cell(row, column).square(),
                    grid.cell(toRow, toColumn)));
    }
  }
